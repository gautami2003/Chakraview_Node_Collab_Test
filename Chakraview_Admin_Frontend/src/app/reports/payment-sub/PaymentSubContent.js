...
function PaymentSubContent() {
  ...
  useEffect(() => {
    const fetchLogs = async () => {
      try {
        ...
        const result = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/paymentSub`, {
          headers: {
        ...
        console.error('Error fetching payment logs:', err?.response?.data || err?.message || err);
        setError(err);
      } finally {
        setLoading(false);
      }
  ...
  const filteredLogs = useMemo(() => {
    return allLogs.filter(log => {
      ...
      const logDate = log.initiatedt ? new Date(log.initiatedt) : null;
  ...
  const fmtinitiatedt = v => (v ? new Date(v).toLocaleString() : '-');
...
>
  <thead className="card-header">
    <tr>
      ...
      <th style={{ width: '15%', wordBreak: 'break-word' }}>Name</th>
      <th style={{ width: '15%', wordBreak: 'break-word' }}>Mobile</th>
      <th style={{ width: '25%', wordBreak: 'break-word' }}>Email</th>
      <th style={{ width: '20%' }}>Initiated Date</th>
    </tr>
  </thead>
  <tbody>
    {currentLogs.map((row, index) => (
      <tr key={`${row.subno}-${index}`}>
        ...
        <td>{fmtinitiatedt(row.initiatedt)}</td>
      </tr>
    ))}
  </tbody>
</table>
...
export default PaymentSubContent;
