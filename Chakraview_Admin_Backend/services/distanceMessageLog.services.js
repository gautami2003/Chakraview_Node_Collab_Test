...
const getAllDistanceMessageLogReport = async (busOperatorId) => {
  if (!busOperatorId) throw new Error("Bus Operator ID is required");

  const rows = await DISTANCE_MESSAGE_LOG.findAll({
    ...
    include: [
      ...
      { model: STOPPAGE_MASTER, as: "stoppage_master", attributes: ["StopageName", "StopageName"] },
      {
        model: DRIVER_ROUTE_TRANSACTION,
        as: "driver_route_transaction",
        attributes: ["DriverRouteTransactionID", "DateTime", "Type", "RouteID", "BusOperatorID"],
        include: [
          { model: BUS_INCHARGE_MASTER, as: "bus_incharge_master", attributes: ["DriverName"] },
        ],
      },
    ],
    order: [["DateTime", "DESC"]],
