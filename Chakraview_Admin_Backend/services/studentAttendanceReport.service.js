...
const { STUDENT_ATTENDANCE, SCHOOL_MASTER, STUDENT_MASTER , PICKUP_ROUTE_MASTER, DROP_ROUTE_MASTER} = DB_MODELS;


const getAllStudentAttendance = async (busOperatorId) => {
  if (!busOperatorId) {
    throw new Error("Bus Operator ID is required");
  }

  const studentAttendance = await STUDENT_ATTENDANCE.findAll({
